...
function CalendarModule({ manager, company }) {
  ...
  async function save(event) { event.preventDefault(); setCalendarError(""); try { const { publication } = await api(`/api/calendar/publications/${editing.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editing) }); setItems((current) => (current.some((item) => item.id === publication.id) ? current.map((item) => item.id === publication.id ? publication : item) : [...current, publication]).sort((a, b) => `${a.date}${a.time || ""}`.localeCompare(`${b.date}${b.time || ""}`))); setEditing(null); } catch (error) { setCalendarError(error.message); } }
  async function remove(id) { setCalendarError(""); try { await api(`/api/calendar/publications/${id}`, { method: "DELETE" }); setItems((current) => current.filter((item) => item.id !== id)); } catch (error) { setCalendarError(error.message); } }
  return <section className="calendar-module">
    ...
    {calendarError && <div className="calendar-error"><AlertTriangle />{calendarError}</div>}{loadingCalendar ? <div className="calendar-loading"><RefreshCw /> Cargando calendario…</div> : items.length ? <div className={`publication-grid ${preview ? "client-view" : ""}`}>{items.map((item) => <article className="publication-card" key={item.id}>
      ...
      <div className="publication-body"><div className="publication-date"><CalendarDays />{item.date ? new Date(`${item.date}T12:00:00`).toLocaleDateString("es-EC", { day: "2-digit", month: "short", year: "numeric" }) : "Sin fecha"}{item.time && ` · ${item.time}`}</div><h3>{item.topic}</h3><p>{item.copy || "Sin copy agregado."}</p><div className="platform-tags">{item.platforms.map((platform) => <span key={platform}>{platform}</span>)}</div>{item.mediaName && !item.mediaUrl && <small className="media-reference"><FileImage />{item.mediaName}</small>}</div>
  ...
  return <main className={`role-portal ${manager ? "manager-portal" : "client-portal"}`}>
    ...
    <section className="portal-workspace"><header><div><small>{manager ? "OPERACIÓN DE MARKETING" : "SEGUIMIENTO DE MARKETING"}</small><b>{portalModules.find((item) => item.id === section)?.label}</b></div><div className="portal-profile"><i>{user.name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</i><span><b>{user.name}</b><small>{manager ? "Gestor" : "Cliente"}</small></span></div></header>
      <div className="portal-content">
        ...
        {section === "calendar" && <CalendarModule manager={manager} company={user.company_name} />}
