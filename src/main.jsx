...
import { AlertTriangle, ArrowLeft, ArrowRight, Building2, CalendarDays, Check, CheckCircle2, ChevronDown, Clock3, Copy, Dices, Download, Eye, EyeOff, FileDown, FileImage, Image, LayoutDashboard, LockKeyhole, LogOut, Mail, Menu, Pencil, Plus, RefreshCw, Save, Search, ShieldCheck, Smartphone, Trash2, Upload, UserPlus, UserRound, Users, Video, X } from "lucide-react";
...
function ActivityModule({ activities }) {
  const actionLabels = { "publication.saved": "Publicación guardada", "publication.approved": "Publicación aprobada", "publication.changes_requested": "Cambios solicitados", "calendar.slots_generated": "Espacios del calendario generados", "content_plan.saved": "Estrategia mensual guardada", "user.created": "Usuario creado", "user.updated": "Usuario actualizado", "user.deleted": "Usuario eliminado", "company.created": "Empresa creada", "company.updated": "Empresa actualizada" };
  const targetLabels = { publication: "Publicación", company: "Empresa", user: "Usuario" };
  return <div className="users-module"><div className="users-heading"><div><span className="dashboard-intro">Registro de auditoría</span><h1>Actividad</h1><p>Últimos eventos de seguridad y gestión registrados por el servidor.</p></div></div><section className="activity-log">{activities.length ? activities.map((item) => <article key={item.id}><ShieldCheck /><div><b>{actionLabels[item.action] || item.action.replaceAll(".", " · ")}</b><span>{item.actor_name || "Usuario eliminado"} · {new Date(item.created_at).toLocaleString("es-EC")}</span></div><small title={`${targetLabels[item.target_type] || item.target_type} #${item.target_id}`}>{targetLabels[item.target_type] || item.target_type} <i>#{item.target_id}</i></small></article>) : <div className="users-empty"><ShieldCheck /><h3>Sin actividad registrada</h3><p>Los nuevos eventos aparecerán aquí.</p></div>}</section></div>;
}
...
function AdminPanel({ user }) {
  ...
  useEffect(() => { api("/api/admin/users").then((result) => setUsers(result.users)).catch((error) => showToast(error.message, "error")); }, []);
  useEffect(() => { if (section === "activity") api("/api/admin/activity").then((result) => setActivities(result.activities)).catch((error) => showToast(error.message, "error")); }, [section]);
  useEffect(() => {
  ...
  function saveUser(savedUser) { setUsers((current) => current.some((item) => item.id === savedUser.id) ? current.map((item) => item.id === savedUser.id ? savedUser : item) : [savedUser, ...current]); setUserModal(null); showToast("Usuario guardado correctamente.", "success"); }
  async function deleteUser(selectedUser) { const confirmed = await confirmAction({ title: "Eliminar usuario", message: `Se eliminará definitivamente la cuenta de ${selectedUser.name} y se cerrarán sus sesiones activas.`, confirmLabel: "Eliminar usuario", danger: true }); if (!confirmed) return; try { await api(`/api/admin/users/${selectedUser.id}`, { method: "DELETE" }); setUsers((current) => current.filter((item) => item.id !== selectedUser.id)); showToast("Usuario eliminado.", "success"); } catch (error) { showToast(error.message, "error"); } }
...
const platformMarks = { Instagram: "IG", Facebook: "f", TikTok: "TT", LinkedIn: "in", YouTube: "▶" };
function CalendarContentCard({ item, onSelect }) {
  const status = item.approvalStatus === "approved" ? "Aprobado" : item.approvalStatus === "changes_requested" ? "Cambios" : "Pendiente";
  return <button type="button" className={`calendar-content-card format-${item.format} ${item.distributionType || "organic"}`} onClick={() => onSelect(item)} title={`Abrir ${item.topic}`}><span className="calendar-card-top"><em>{item.format}</em><i className={`calendar-card-status ${item.approvalStatus || "pending"}`}></i></span><strong>{item.topic}</strong><span className="calendar-card-meta">{item.time && <small><Clock3 />{item.time}</small>}<small>{item.distributionType === "paid" ? "Pauta" : "Orgánico"}</small></span><span className="calendar-platforms">{(item.platforms || []).map((platform) => <i className={`platform-${platform.toLowerCase()}`} key={platform} title={platform} aria-label={platform}><span>{platformMarks[platform] || platform.slice(0, 2)}</span></i>)}{!(item.platforms || []).length && <small>Sin plataforma</small>}<small className="calendar-status-label">{status}</small></span></button>;
}
...
function CalendarModule({ manager, company }) {
  ...
  async function save(event) { event.preventDefault(); if (savingContent) return; setSavingContent(true); setCalendarError(""); try { const { publication } = await api(`/api/calendar/publications/${editing.id}`, { method: "PUT", headers: { "Content-Type": "application/json", "X-Focugex-Company": company }, body: JSON.stringify(editing) }); setItems((current) => (current.some((item) => item.id === publication.id) ? current.map((item) => item.id === publication.id ? publication : item) : [...current, publication]).sort((a, b) => `${a.date}${a.time || ""}`.localeCompare(`${b.date}${b.time || ""}`))); setEditing(null); showToast("Contenido guardado en el calendario.", "success"); } catch (error) { setCalendarError(error.message); } finally { setSavingContent(false); } }
  async function remove(id) { const confirmed = await confirmAction({ title: "Eliminar contenido", message: "Esta publicación desaparecerá del calendario y del próximo PDF.", confirmLabel: "Eliminar", danger: true }); if (!confirmed) return; setCalendarError(""); try { await api(`/api/calendar/publications/${id}`, { method: "DELETE", headers: { "X-Focugex-Company": company } }); setItems((current) => current.filter((item) => item.id !== id)); showToast("Contenido eliminado.", "success"); } catch (error) { setCalendarError(error.message); } }
  async function review(id, status) { if (reviewingContent) return; const comment = status === "changes_requested" ? await requestText({ title: "Solicitar cambios", message: "Explica con claridad qué debe ajustar el gestor.", label: "Comentario para el gestor", placeholder: "Ej. Ajustar el texto y reemplazar la imagen…", confirmLabel: "Enviar solicitud" }) : ""; if (status === "changes_requested" && !comment?.trim()) return; setReviewingContent(true); setCalendarError(""); try { const { review: result } = await api(`/api/calendar/publications/${id}/review`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status, comment }) }); setItems((current) => current.map((item) => item.id === id ? { ...item, ...result } : item)); setSelectedContent((current) => current?.id === id ? { ...current, ...result } : current); showToast(status === "approved" ? "Contenido aprobado." : "Cambios enviados al gestor.", "success"); } catch (error) { setCalendarError(error.message); } finally { setReviewingContent(false); } }
  return <section className="calendar-module">
    ...
    {preview && viewMode === "month" && <CalendarPresentation plan={plan} items={presentationItems} period={period} company={company} onSelectContent={(item) => setSelectedContent(item)} />}
    {preview && viewMode === "week" && <WeekPresentation items={presentationItems} selectedDate={selectedDate} onSelectContent={(item) => setSelectedContent(item)} />}
    {preview && viewMode === "day" && <DayPresentation items={presentationItems} selectedDate={selectedDate} company={company} />}
    ...
    {editing && <div className="publication-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setEditing(null)}>
      <section className="publication-modal" role="dialog" aria-modal="true">
        ...
        <form onSubmit={save}>
          ...
          <footer><button type="button" onClick={() => setEditing(null)} disabled={savingContent}>Cancelar</button><button className="save-publication" disabled={savingContent}><Save /> {savingContent ? "Guardando…" : "Guardar contenido"}</button></footer>
        </form>
      </section>
    </div>}
...
function CompaniesModule({ companies, clients, owner, onClientCreated, onCompanyCreated, onCompanyUpdated }) {
  ...
  async function createCompany() { const companyData = await requestCompanyName(); if (!companyData) return; try { const result = await api("/api/manager/companies", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(companyData) }); onCompanyCreated(result.company); } catch (requestError) { showAppNotice(requestError.message); } }
  async function editCompany(company) { const companyData = await requestCompanyName(company); if (!companyData) return; try { const result = await api(`/api/manager/companies/${company.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(companyData) }); onCompanyUpdated(company.name, result.company); } catch (requestError) { showAppNotice(requestError.message); } }
  function addClient(companyName) { setForm({ name: "", username: "", email: "", companyName, password: "" }); setOpen(true); }
  return <section className="manager-clients"><header><div><span className="eyebrow">CARTERA DE LA AGENCIA</span><h2>Empresas</h2><p>Crea las empresas cuyo marketing administra tu agencia y sus accesos de cliente.</p></div>{owner && <button onClick={createCompany}><Plus /> Nueva empresa</button>}</header><div className="company-management-grid">{companies.map((company) => <article key={company.id}><header>{company.logoData ? <img className="company-card-logo" src={company.logoData} alt={`Logo de ${company.name}`} /> : <Building2 />}<div><b>{company.name}</b><span>{clients.filter((client) => client.company_name.toLowerCase() === company.name.toLowerCase()).length} usuario(s) cliente</span></div><div className="company-card-actions">{owner && <button onClick={() => editCompany(company)}><Pencil /> Editar empresa</button>}<button onClick={() => addClient(company.name)}><UserPlus /> Agregar usuario</button></div></header><div className="company-users">{clients.filter((client) => client.company_name.toLowerCase() === company.name.toLowerCase()).map((client) => <div key={client.id}><i>{client.name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</i><span><b>{client.name}</b><small>{client.email}</small></span><em className={client.active ? "active" : ""}>{client.active ? "Activo" : "Suspendido"}</em></div>)}{!clients.some((client) => client.company_name.toLowerCase() === company.name.toLowerCase()) && <p>Esta empresa todavía no tiene usuarios cliente.</p>}</div></article>)}</div>{!companies.length && <div className="calendar-empty"><Building2 /><h3>No hay empresas administradas</h3><p>El propietario de la agencia debe crear la primera empresa.</p></div>}{open && <div className="publication-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}><section className="publication-modal client-create-modal"><header><div><span className="eyebrow">USUARIO DE EMPRESA</span><h2>Crear acceso de cliente</h2></div><button onClick={() => setOpen(false)}><X /></button></header><form onSubmit={submit}><div className="publication-form-grid"><label>Nombre completo<input value={form.name} onChange={(e) => update("name", e.target.value)} required /></label><label>Nombre de usuario<input value={form.username} onChange={(e) => update("username", e.target.value.toLowerCase().replace(/\s/g, ""))} required /></label><label>Correo electrónico<input type="email" value={form.email} onChange={(e) => update("email", e.target.value)} required /></label><label>Contraseña<input type="password" minLength="10" value={form.password} onChange={(e) => update("password", e.target.value)} placeholder="10 caracteres, mayúscula y número" required /></label><label className="wide">Empresa<input value={form.companyName} readOnly /></label></div>{error && <div className="calendar-error"><AlertTriangle />{error}</div>}<footer><button type="button" onClick={() => setOpen(false)}>Cancelar</button><button className="save-publication" disabled={saving}><UserPlus />{saving ? "Creando…" : "Crear usuario cliente"}</button></footer></form></section></div>}</section>;
}
...
function CollaboratorsModule({ collaborators, owner, onCreated }) {
  ...
  async function submit(event) {
    ...
    try { const result = await api("/api/manager/collaborators", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) }); onCreated(result.collaborator); setOpen(false); setForm({ name: "", username: "", email: "", password: "" }); showToast("Colaborador creado correctamente.", "success"); }
    catch (requestError) { setError(requestError.message); }
    finally { setSaving(false); }
  }
  return <section className="manager-clients"><header><div><span className="eyebrow">EQUIPO INTERNO</span><h2>Colaboradores</h2><p>Miembros de tu agencia que pueden trabajar en las empresas y calendarios asignados.</p></div>{owner && <button onClick={() => setOpen(true)}><UserPlus /> Nuevo colaborador</button>}</header><div className="manager-client-grid">{collaborators.map((item) => <article key={item.id}><i>{item.name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</i><div><b>{item.name}</b><span>{item.email}</span><small>@{item.username}</small></div><em className={item.active ? "active" : ""}>{item.active ? "Activo" : "Suspendido"}</em></article>)}</div>{!collaborators.length && <div className="calendar-empty"><Users /><h3>Sin colaboradores</h3><p>El propietario de la agencia puede crear miembros internos.</p></div>}{open && <div className="publication-modal-backdrop"><section className="publication-modal"><header><h2>Nuevo colaborador</h2><button onClick={() => !saving && setOpen(false)} aria-label="Cerrar"><X /></button></header><form onSubmit={submit}><div className="publication-form-grid"><label>Nombre<input value={form.name} onChange={(e) => update("name", e.target.value)} required /></label><label>Usuario<input value={form.username} onChange={(e) => update("username", e.target.value)} required /></label><label>Correo<input type="email" value={form.email} onChange={(e) => update("email", e.target.value)} required /></label><label>Contraseña<input type="password" minLength="10" value={form.password} onChange={(e) => update("password", e.target.value)} required /></label></div>{error && <div className="calendar-error">{error}</div>}<footer><button type="button" disabled={saving} onClick={() => setOpen(false)}>Cancelar</button><button className="save-publication" disabled={saving}>{saving ? "Creando…" : "Crear colaborador"}</button></footer></form></section></div>}</section>;
}

function MonthStructureModule({ company }) {
  const [period, setPeriod] = useState(() => new Date().toISOString().slice(0, 7));
  const [form, setForm] = useState({ strategySummary: "", postsPerWeek: 0, postsDetail: "", videosPerMonth: 0, videosDetail: "", videoSchedule: "", videoBoostDetail: "", mainLinesCount: 0, mainLines: "", publicationIntervalDays: 0, keyDates: [] });
  const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [message, setMessage] = useState("");
  useEffect(() => { let active = true; setLoading(true); setMessage(""); api(`/api/calendar/plan?period=${period}`, { headers: { "X-Focugex-Company": company } }).then(({ plan }) => active && setForm({ strategySummary: "", postsPerWeek: 0, postsDetail: "", videosPerMonth: 0, videosDetail: "", videoSchedule: "", videoBoostDetail: "", mainLinesCount: 0, mainLines: "", publicationIntervalDays: 0, keyDates: [], ...(plan || {}) })).catch((error) => active && setMessage(error.message)).finally(() => active && setLoading(false)); return () => { active = false; }; }, [company, period]);
  function update(field, value) { setForm((current) => ({ ...current, [field]: value })); }
  function updateKeyDate(index, field, value) { setForm((current) => ({ ...current, keyDates: current.keyDates.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: value } : item) })); }
  function addKeyDate() { setForm((current) => ({ ...current, keyDates: [...current.keyDates, { date: `${period}-01`, title: "", description: "" }] })); }
  function removeKeyDate(index) { setForm((current) => ({ ...current, keyDates: current.keyDates.filter((_, itemIndex) => itemIndex !== index) })); }
  async function save(event) { event.preventDefault(); setSaving(true); setMessage(""); try { const { plan } = await api("/api/calendar/plan", { method: "PUT", headers: { "Content-Type": "application/json", "X-Focugex-Company": company }, body: JSON.stringify({ ...form, period }) }); const generated = await api("/api/calendar/generate-slots", { method: "POST", headers: { "Content-Type": "application/json", "X-Focugex-Company": company }, body: JSON.stringify({ period }) }); setForm((current) => ({ ...current, ...plan })); setMessage(`Estructura guardada y ${generated.count} espacios editables creados en el calendario.`); } catch (error) { setMessage(error.message); } finally { setSaving(false); } }
  return <section className="structure-module"><header><div><span className="eyebrow">PLANIFICACIÓN GUIADA</span><h2>Estructura del mes</h2><p>Responde las preguntas para construir el PDF y crear borradores editables en el calendario.</p></div><input className="period-picker" type="month" value={period} onChange={(event) => setPeriod(event.target.value)} /></header>{loading ? <div className="calendar-loading"><RefreshCw /> Cargando estructura…</div> : <form onSubmit={save}><label className="structure-wide"><span>1. ¿Cómo resumirías la planificación de este mes?</span><textarea rows="3" value={form.strategySummary || ""} onChange={(event) => update("strategySummary", event.target.value)} /></label><label><span>2. ¿Cuántos espacios para posts habrá por semana?</span><input type="number" min="0" max="30" value={form.postsPerWeek || 0} onChange={(event) => update("postsPerWeek", Number(event.target.value))} /></label><label><span>3. ¿Cómo se distribuyen esos posts?</span><input value={form.postsDetail || ""} onChange={(event) => update("postsDetail", event.target.value)} /></label><label><span>4. ¿Cuántos videos tendrá el mes?</span><input type="number" min="0" max="100" value={form.videosPerMonth || 0} onChange={(event) => update("videosPerMonth", Number(event.target.value))} /></label><label><span>5. ¿Qué tipos de videos se producirán?</span><input value={form.videosDetail || ""} onChange={(event) => update("videosDetail", event.target.value)} /></label><label><span>6. ¿En qué fechas se pautarán los videos?</span><input value={form.videoSchedule || ""} onChange={(event) => update("videoSchedule", event.target.value)} /></label><label><span>7. ¿Cuál será la frecuencia o intención de la pauta?</span><input value={form.videoBoostDetail || ""} onChange={(event) => update("videoBoostDetail", event.target.value)} /></label><label><span>8. ¿Cuántas líneas principales habrá?</span><input type="number" min="0" max="100" value={form.mainLinesCount || 0} onChange={(event) => update("mainLinesCount", Number(event.target.value))} /></label><label><span>9. ¿Cuáles son esas líneas de contenido?</span><textarea rows="4" value={form.mainLines || ""} onChange={(event) => update("mainLines", event.target.value)} /></label><label className="structure-wide"><span>10. ¿Cada cuántos días debe existir una publicación?</span><input type="number" min="1" max="31" value={form.publicationIntervalDays || 0} onChange={(event) => update("publicationIntervalDays", Number(event.target.value))} placeholder="Ej. cada 2 días" /></label><fieldset className="key-dates"><legend>11. Fechas específicas del mes</legend><p>Agrega celebraciones, campañas o contenidos que deben publicarse en un día exacto.</p>{form.keyDates.map((item, index) => <div key={`${item.date}-${index}`}><input type="date" min={`${period}-01`} max={`${period}-31`} value={item.date} onChange={(event) => updateKeyDate(index, "date", event.target.value)} required /><input value={item.title} onChange={(event) => updateKeyDate(index, "title", event.target.value)} placeholder="Ej. Día de la Bandera" required /><input value={item.description} onChange={(event) => updateKeyDate(index, "description", event.target.value)} placeholder="Descripción o intención del contenido" /><button type="button" onClick={() => removeKeyDate(index)} aria-label="Eliminar fecha"><Trash2 /></button></div>)}<button type="button" className="add-key-date" onClick={addKeyDate}><Plus /> Agregar fecha específica</button></fieldset><footer><span className={message.startsWith("Estructura guardada") ? "success" : ""}>{message}</span><button className="save-publication" disabled={saving}><Save />{saving ? "Guardando…" : "Guardar y crear espacios"}</button></footer></form>}</section>;
}
...
function CalendarPdfModule({ company }) {
  ...
  async function generatePreview() {
    ...
    try {
      ...
      const [publicationResult, planResult, companyResult] = await Promise.all([api("/api/calendar/publications", { headers }), api(`/api/calendar/plan?period=${period}`, { headers }), api("/api/company/current", { headers })]);
      const calendarItems = publicationResult.publications.filter((item) => item.date?.startsWith(period));
      const publications = calendarItems.filter((item) => !item.isDraftSlot);
      if (!calendarItems.length && !planResult.plan?.keyDates?.length) throw new Error("No hay publicaciones ni fechas específicas guardadas para el mes seleccionado.");
      const document = await createCalendarPdf({ company, logoData: companyResult.company.logoData, period, plan: planResult.plan, publications, calendarItems });
...
function RolePortal({ user }) {
  ...
  useEffect(() => { if (!manager) return; Promise.all([api("/api/manager/companies"), api("/api/manager/clients"), api("/api/manager/collaborators")]).then(([companyResult, clientResult, collaboratorResult]) => { setCompanies(companyResult.companies); setClients(clientResult.clients); setCollaborators(collaboratorResult.collaborators); setActiveCompany((current) => companyResult.companies.some((item) => item.name === current) ? current : companyResult.companies[0]?.name || ""); }).catch((error) => showToast(error.message, "error")); }, [manager]);
  ...
  function companyCreated(company) { setCompanies((current) => current.some((item) => item.id === company.id) ? current.map((item) => item.id === company.id ? { ...item, ...company } : item) : [...current, company]); setActiveCompany(company.name); }
  function companyUpdated(previousName, company) { setCompanies((current) => current.map((item) => item.id === company.id ? { ...item, ...company } : item)); setClients((current) => current.map((client) => client.company_name.toLowerCase() === previousName.toLowerCase() ? { ...client, company_name: company.name } : client)); setActiveCompany((current) => current.toLowerCase() === previousName.toLowerCase() ? company.name : current); }
  async function logout() { await api("/api/auth/logout", { method: "POST" }); window.location.assign("/"); }
  const modules = manager ? [...portalModules, { id: "month-structure", label: "Estructura del mes", icon: LayoutDashboard }, { id: "companies", label: "Empresas", icon: Building2 }, { id: "collaborators", label: "Colaboradores", icon: Users }] : portalModules;
  return <main className={`role-portal ${manager ? "manager-portal" : "client-portal"}`}>
    ...
    <section className="portal-workspace"><header><div><small>{manager ? "OPERACIÓN DE MARKETING" : "SEGUIMIENTO DE MARKETING"}</small><b>{modules.find((item) => item.id === section)?.label}</b></div><div className="portal-header-actions">{manager && <label className="company-switcher"><Building2 /><span><small>EMPRESA ACTIVA</small><select value={activeCompany} onChange={(e) => setActiveCompany(e.target.value)}>{companies.map((company) => <option key={company.id} value={company.name}>{company.name}</option>)}</select></span></label>}<div className="portal-profile"><i>{user.name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</i><span><b>{user.name}</b><small>{manager ? "Gestor" : "Cliente"}</small></span></div></div></header>
      <div className="portal-content">
        ...
        {section === "month-structure" && <MonthStructureModule company={activeCompany} />}
        {section === "companies" && <CompaniesModule companies={companies} clients={clients} owner={owner} onClientCreated={clientCreated} onCompanyCreated={companyCreated} onCompanyUpdated={companyUpdated} />}
