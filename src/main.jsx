...
function CalendarModule({ manager, company }) {
  ...
  function openEditor(item = null) { setEditing(item ? { ...item } : { ...emptyPublication, id: crypto.randomUUID() }); }
  ...
  async function review(id, status) { const comment = status === "changes_requested" ? window.prompt("Describe los cambios que necesita el gestor:") : ""; if (status === "changes_requested" && !comment?.trim()) return; setCalendarError(""); try { const { review: result } = await api(`/api/calendar/publications/${id}/review`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status, comment }) }); setItems((current) => current.map((item) => item.id === id ? { ...item, ...result } : item)); } catch (error) { setCalendarError(error.message); } }
  return <section className="calendar-module">
    <header className="calendar-toolbar"><div><span className="eyebrow">PLANIFICACIÓN DE CONTENIDO</span><h2>Calendario editorial</h2><p>{preview ? "Así verá el cliente las publicaciones planificadas." : "Crea y organiza cada publicación antes de enviarla al cliente."}</p></div><div>{manager && <button className={`preview-toggle ${clientPreview ? "active" : ""}`} onClick={() => setClientPreview(!clientPreview)}><Eye />{clientPreview ? "Volver al editor" : "Vista del cliente"}</button>}{manager && !preview && <button className="primary-action" onClick={() => openEditor()}><Plus /> Nueva publicación</button>}</div></header>
    {calendarError && <div className="calendar-error"><AlertTriangle />{calendarError}</div>}{loadingCalendar ? <div className="calendar-loading"><RefreshCw /> Cargando calendario…</div> : items.length ? <div className={`publication-grid ${preview ? "client-view" : ""}`}>{items.map((item) => <article className="publication-card" key={item.id}>
      ...
      <div className="publication-body"><div className="publication-date"><CalendarDays />{item.date ? new Date(`${item.date}T12:00:00`).toLocaleDateString("es-EC", { day: "2-digit", month: "short", year: "numeric" }) : "Sin fecha"}{item.time && ` · ${item.time}`}</div><h3>{item.topic}</h3><p>{item.copy || "Sin copy agregado."}</p><div className="platform-tags">{item.platforms.map((platform) => <span key={platform}>{platform}</span>)}</div>{item.mediaName && !item.mediaUrl && <small className="media-reference"><FileImage />{item.mediaName}</small>}<div className={`approval-state ${item.approvalStatus || "pending"}`}>{item.approvalStatus === "approved" ? "Aprobado por el cliente" : item.approvalStatus === "changes_requested" ? "Cambios solicitados" : "Pendiente de aprobación"}</div>{item.clientComment && <blockquote className="client-comment">{item.clientComment}</blockquote>}</div>
    ...
    </article>)}</div> : <div className="calendar-empty"><CalendarDays /><h3>No hay publicaciones planificadas</h3><p>{manager ? "Crea el primer cuadro del calendario con su fecha, copy y material." : "El equipo de marketing todavía no ha compartido publicaciones."}</p>{manager && !preview && <button className="primary-action" onClick={() => openEditor()}><Plus /> Crear primera publicación</button>}</div>}
    {editing && <div className="publication-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setEditing(null)}><section className="publication-modal" role="dialog" aria-modal="true"><header><div><span className="eyebrow">EDITOR DE CALENDARIO</span><h2>{items.some((item) => item.id === editing.id) ? "Editar publicación" : "Nueva publicación"}</h2></div><button onClick={() => setEditing(null)} aria-label="Cerrar"><X /></button></header><form onSubmit={save}><div className="publication-form-grid"><label>Fecha<input type="date" value={editing.date} onChange={(e) => update("date", e.target.value)} required /></label><label>Hora<input type="time" value={editing.time} onChange={(e) => update("time", e.target.value)} /></label><label className="wide">Tema<input value={editing.topic} onChange={(e) => update("topic", e.target.value)} placeholder="Ej. Lanzamiento de temporada" required /></label><label className="wide">Copy<textarea value={editing.copy} onChange={(e) => update("copy", e.target.value)} placeholder="Escribe el texto de la publicación…" rows="5" /></label></div><fieldset><legend>Tipo de contenido</legend><div className="format-options">{["post", "reel", "historia"].map((format) => <button type="button" className={editing.format === format ? "selected" : ""} key={format} onClick={() => update("format", format)}>{format === "reel" ? <Video /> : format === "historia" ? <Smartphone /> : <FileImage />}{format}</button>)}</div></fieldset><fieldset><legend>Plataformas <small>Puedes elegir varias</small></legend><div className="platform-options">{platformOptions.map((platform) => <label key={platform}><input type="checkbox" checked={editing.platforms.includes(platform)} onChange={() => togglePlatform(platform)} /><span><Check /></span>{platform}</label>)}</div></fieldset><label className="media-upload"><input type="file" accept="image/*,video/*" onChange={selectMedia} /><Upload /><span><b>Agregar imagen o video</b><small>{editing.mediaName || "JPG, PNG, WEBP, MP4 o MOV"}</small></span></label>{editing.mediaUrl && <div className="media-preview">{editing.mediaType === "video" ? <video src={editing.mediaUrl} controls /> : <img src={editing.mediaUrl} alt="Vista previa" />}</div>}<footer><button type="button" onClick={() => setEditing(null)}>Cancelar</button><button className="save-publication"><Save /> Guardar publicación</button></footer></form></section></div>}
  </section>;
}

function ManagerClients({ companies, clients, onCreated }) {
...
function RolePortal({ user }) {
  ...
  useEffect(() => { if (!manager) return; Promise.all([api("/api/manager/companies"), api("/api/manager/clients")]).then(([companyResult, clientResult]) => { setCompanies(companyResult.companies); setClients(clientResult.clients); setActiveCompany((current) => companyResult.companies.some((item) => item.name === current) ? current : companyResult.companies[0]?.name || ""); }).catch(() => {}); }, [manager]);
  ...
  async function logout() { await api("/api/auth/logout", { method: "POST" }); window.location.assign("/"); }
  const modules = manager ? [...portalModules, { id: "clients", label: "Clientes", icon: Users }] : portalModules;
