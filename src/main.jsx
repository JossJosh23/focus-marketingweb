...
function CalendarModule({ manager, company }) {
  const storageKey = `focugex_calendar_${(company || "general").toLowerCase().replace(/[^a-z0-9]/g, "_")}`;
  ...
  useEffect(() => { localStorage.setItem(storageKey, JSON.stringify(items.map(({ mediaUrl, ...item }) => ({ ...item, mediaUrl: mediaUrl?.startsWith("blob:") ? "" : mediaUrl })))); }, [items, storageKey]);
  ...
  function selectMedia(event) { const file = event.target.files[0]; if (!file) return; setEditing((current) => ({ ...current, mediaUrl: URL.createObjectURL(file), mediaType: file.type.startsWith("video/") ? "video" : "image", mediaName: file.name })); }
  function save(event) { event.preventDefault(); setItems((current) => current.some((item) => item.id === editing.id) ? current.map((item) => item.id === editing.id ? editing : item) : [...current, editing].sort((a, b) => `${a.date}${a.time}`.localeCompare(`${b.date}${b.time}`))); setEditing(null); }
  ...
  return <section className="calendar-module">
    ...
    {items.length ? <div className={`publication-grid ${preview ? "client-view" : ""}`}>{items.map((item) => <article className="publication-card" key={item.id}>
