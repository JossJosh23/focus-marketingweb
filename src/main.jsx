...
function CalendarModule({ manager, company }) {
  ...
  return <section className="calendar-module">
    <header className="calendar-toolbar"><div><span className="eyebrow">PLANIFICACIÓN DE CONTENIDO</span><h2>Calendario editorial</h2><p>{preview ? "Así verá el cliente las publicaciones planificadas." : "Crea y organiza cada publicación antes de enviarla al cliente."}</p></div><div><input className="period-picker" type="month" value={period} onChange={(e) => setPeriod(e.target.value)} />{manager && !preview && <button onClick={() => setPlanOpen(true)}><Pencil /> Estrategia mensual</button>}{manager && <button className={`preview-toggle ${clientPreview ? "active" : ""}`} onClick={() => setClientPreview(!clientPreview)}><Eye />{clientPreview ? "Volver al editor" : "Vista del cliente"}</button>}{manager && !preview && <button className="primary-action" onClick={() => openEditor()}><Plus /> Nueva publicación</button>}</div></header>
    {preview && <CalendarPresentation plan={plan} items={periodItems} period={period} company={company} />}
