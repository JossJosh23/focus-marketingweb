...
function RolePortal({ user }) {
  ...
  return <main className={`role-portal ${manager ? "manager-portal" : "client-portal"}`}>
    <aside className="portal-sidebar"><Logo /><span className="portal-role">{manager ? "GESTOR DE MARKETING" : "PORTAL DEL CLIENTE"}</span><nav>{modules.map(({ id, label, icon: Icon }) => <button key={id} className={section === id ? "active" : ""} onClick={() => setSection(id)}><Icon />{label}</button>)}</nav><div className="portal-company"><Building2 /><span><small>ESPACIO DE TRABAJO</small><b>{activeCompany || user.company_name || "FOCUGEX"}</b></span></div><button className="portal-logout" onClick={logout}><LogOut /> Cerrar sesión</button></aside>
