...
function CalendarPresentation({ plan, items, period, company }) {
...
function WeekPresentation({ items, selectedDate }) {
  ...
  return <section className="week-presentation"><header><span>SEMANA DEL {monday.toLocaleDateString("es-EC", { day: "2-digit", month: "long" }).toUpperCase()}</span></header><div>{dates.map((date) => { const key = date.toISOString().slice(0, 10); const entries = items.filter((item) => item.date === key); return <article key={key}><header><small>{date.toLocaleDateString("es-EC", { weekday: "short" })}</small><b>{date.getDate()}</b></header>{entries.map((item) => <div className={item.distributionType || "organic"} key={item.id}><span>{item.format}{item.distributionType === "paid" ? " · pauta" : ""}</span><b>{item.topic}</b><small>{item.time || "Sin hora"}</small></div>)}{!entries.length && <p>Sin contenido</p>}</article>; })}</div></section>;
}
...
function CalendarModule({ manager, company }) {
  ...
  return <section className="calendar-module">
    ...
    {preview && viewMode === "month" && <CalendarPresentation plan={plan} items={periodItems} period={period} company={company} />}
    {preview && viewMode === "week" && <WeekPresentation items={periodItems} selectedDate={selectedDate} />}
    {preview && viewMode === "day" && <DayPresentation items={periodItems} selectedDate={selectedDate} company={company} />}
    {calendarError && <div className="calendar-error"><AlertTriangle />{calendarError}</div>}{loadingCalendar ? <div className="calendar-loading"><RefreshCw /> Cargando calendario…</div> : periodItems.length ? <div className={`publication-grid ${preview ? "client-view" : ""}`}>{periodItems.map((item) => <article className="publication-card" key={item.id}>
