...
function CalendarPdfModule({ company }) {
  ...
  async function generatePreview() {
    ...
    try {
      ...
      const publications = publicationResult.publications.filter((item) => item.date?.startsWith(period) && !item.isDraftSlot);
      if (!publications.length && !planResult.plan?.keyDates?.length) throw new Error("No hay publicaciones ni fechas específicas guardadas para el mes seleccionado.");
      const document = await createCalendarPdf({ company, logoData: companyResult.company.logoData, period, plan: planResult.plan, publications });
