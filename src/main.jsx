...
function CompaniesModule({ companies, clients, owner, onClientCreated, onCompanyCreated }) {
  ...
  async function createCompany() { const name = window.prompt("Nombre de la empresa que administrará la agencia:"); if (!name?.trim()) return; try { const result = await api("/api/manager/companies", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name }) }); onCompanyCreated(result.company); } catch (requestError) { window.alert(requestError.message); } }
