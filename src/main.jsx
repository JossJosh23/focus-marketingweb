...
function MonthStructureModule({ company }) {
  const [period, setPeriod] = useState(() => new Date().toISOString().slice(0, 7));
  const [form, setForm] = useState({ strategySummary: "", postsPerWeek: 0, postsDetail: "", videosPerMonth: 0, videosDetail: "", videoSchedule: "", videoBoostDetail: "", mainLinesCount: 0, mainLines: "" });
  const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [message, setMessage] = useState("");
  useEffect(() => { let active = true; setLoading(true); setMessage(""); api(`/api/calendar/plan?period=${period}`, { headers: { "X-Focugex-Company": company } }).then(({ plan }) => active && setForm({ strategySummary: "", postsPerWeek: 0, postsDetail: "", videosPerMonth: 0, videosDetail: "", videoSchedule: "", videoBoostDetail: "", mainLinesCount: 0, mainLines: "", ...(plan || {}) })).catch((error) => active && setMessage(error.message)).finally(() => active && setLoading(false)); return () => { active = false; }; }, [company, period]);
  function update(field, value) { setForm((current) => ({ ...current, [field]: value })); }
  async function save(event) { event.preventDefault(); setSaving(true); setMessage(""); try { const { plan } = await api("/api/calendar/plan", { method: "PUT", headers: { "Content-Type": "application/json", "X-Focugex-Company": company }, body: JSON.stringify({ ...form, period }) }); setForm((current) => ({ ...current, ...plan })); setMessage("Estructura guardada. Ya se incluirá en la próxima vista previa del PDF."); } catch (error) { setMessage(error.message); } finally { setSaving(false); } }
  return <section className="structure-module"><header><div><span className="eyebrow">PLANIFICACIÓN GUIADA</span><h2>Estructura del mes</h2><p>Responde las preguntas para construir automáticamente la segunda página del cronograma PDF.</p></div><input className="period-picker" type="month" value={period} onChange={(event) => setPeriod(event.target.value)} /></header>{loading ? <div className="calendar-loading"><RefreshCw /> Cargando estructura…</div> : <form onSubmit={save}><label className="structure-wide"><span>1. ¿Cómo resumirías la planificación de este mes?</span><textarea rows="3" value={form.strategySummary || ""} onChange={(event) => update("strategySummary", event.target.value)} placeholder="Ej. El mes combina contenidos confirmados y espacios editables para completar después." /></label><label><span>2. ¿Cuántos espacios para posts habrá por semana?</span><input type="number" min="0" max="30" value={form.postsPerWeek || 0} onChange={(event) => update("postsPerWeek", Number(event.target.value))} /></label><label><span>3. ¿Cómo se distribuyen esos posts?</span><input value={form.postsDetail || ""} onChange={(event) => update("postsDetail", event.target.value)} placeholder="Ej. 2 orgánicos + 1 para pauta" /></label><label><span>4. ¿Cuántos videos tendrá el mes?</span><input type="number" min="0" max="100" value={form.videosPerMonth || 0} onChange={(event) => update("videosPerMonth", Number(event.target.value))} /></label><label><span>5. ¿Qué tipos de videos se producirán?</span><input value={form.videosDetail || ""} onChange={(event) => update("videosDetail", event.target.value)} placeholder="Ej. 2 reels formales + 2 TikToks informales" /></label><label><span>6. ¿En qué fechas se pautarán los videos?</span><input value={form.videoSchedule || ""} onChange={(event) => update("videoSchedule", event.target.value)} placeholder="Ej. 7, 17 y 27 de septiembre" /></label><label><span>7. ¿Cuál será la frecuencia o intención de la pauta?</span><input value={form.videoBoostDetail || ""} onChange={(event) => update("videoBoostDetail", event.target.value)} placeholder="Ej. Un impulso cada 10 días" /></label><label><span>8. ¿Cuántas líneas principales habrá?</span><input type="number" min="0" max="100" value={form.mainLinesCount || 0} onChange={(event) => update("mainLinesCount", Number(event.target.value))} /></label><label><span>9. ¿Cuáles son esas líneas de contenido?</span><textarea rows="4" value={form.mainLines || ""} onChange={(event) => update("mainLines", event.target.value)} placeholder="Ej. Plato del Mes, cumpleaños, aniversario, web y Día de la Bandera" /></label><footer><span className={message.startsWith("Estructura guardada") ? "success" : ""}>{message}</span><button className="save-publication" disabled={saving}><Save />{saving ? "Guardando…" : "Guardar estructura"}</button></footer></form>}</section>;
}
